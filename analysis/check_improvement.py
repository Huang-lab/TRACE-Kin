#!/usr/bin/env python
"""Human-readable improvement summary for the v2 rerun.

Reads ``data/benchmark/trace_kin_v2_results.csv`` (produced by
``analysis/aggregate_v2_results.py``) and produces a single concise report
answering "did the v2 redesign actually improve over v1, and does it close
...
* **Per-task table** — one row per (dataset, kinetic, split) with v1 RMSE,
  v2 RMSE, RF best, Δv1 = v2−v1, Δrf = v2−rf, and a per-row verdict
  (``beats RF``, ``ties RF``, ``worse than RF``, ``worse than v1``).
* **Per-kinetic aggregate** — mean Δv1, mean Δrf, # splits where v2 ≤ RF,
  per-kinetic verdict.
...
def row_verdict(v1: float | None, v2: float | None, rf: float | None) -> str:
    if v2 is None or pd.isna(v2):
        return "no v2 result"
    if v1 is not None and not pd.isna(v1) and v2 > v1:
        return "✗ worse than v1"
    if rf is None or pd.isna(rf):
        return "no RF reference"
    if v2 <= rf:
        return "✓ beats RF"
    if v2 - rf <= TIE_TOLERANCE:
        return "~ ties RF"
...
def per_kinetic_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for k in sorted(df["k_type"].dropna().unique()):
        sub = df[df["k_type"] == k]
        valid_v1 = sub.dropna(subset=["v1_rmse", "v2_rmse"])
        valid_rf = sub.dropna(subset=["v2_rmse", "rf_best_rmse"])
        beats_rf = (valid_rf["v2_rmse"] <= valid_rf["rf_best_rmse"]).sum()
        n_splits = len(valid_rf)
        # Verdict: same logic as promotion_gate.py — ≥2/3 splits beat RF, OR
        # the per-kinetic mean v2 ≤ mean RF.
        by_count = (n_splits >= 3 and beats_rf >= 2) or (n_splits < 3 and beats_rf == n_splits and n_splits > 0)
        by_mean = (valid_rf["v2_rmse"].mean() <= valid_rf["rf_best_rmse"].mean()) if not valid_rf.empty else False
        ...
        rows.append({
            ...
            "mean_v2": round(valid_rf["v2_rmse"].mean(), 4) if not valid_rf.empty else None,
            "mean_rf": round(valid_rf["rf_best_rmse"].mean(), 4) if not valid_rf.empty else None,
            "mean_delta_v1": round(valid_v1["v2_minus_v1"].mean(), 4) if not valid_v1.empty else None,
            "mean_delta_rf": round(valid_rf["v2_minus_rf"].mean(), 4) if not valid_rf.empty else None,
...
def headline(df: pd.DataFrame, kinetic_summary: pd.DataFrame) -> str:
    ...
    valid = df.dropna(subset=["v1_rmse", "v2_rmse"])
    mean_v1_minus_v2 = (valid["v1_rmse"] - valid["v2_rmse"]).mean() if not valid.empty else 0.0
    valid_rf = df.dropna(subset=["v2_rmse", "rf_best_rmse"])
    n_beats_rf = int((valid_rf["v2_rmse"] <= valid_rf["rf_best_rmse"]).sum())
    n_total_rf = int(len(valid_rf))
    ki_rows = df[df["k_type"] == "ki"].dropna(subset=["v1_rmse", "v2_rmse"])
    ki_max_regress = (ki_rows["v2_rmse"] - ki_rows["v1_rmse"]).max() if not ki_rows.empty else 0.0
    return (
        f"v2 improves over v1 by mean {mean_v1_minus_v2:+.4f} RMSE across "
        f"{len(valid)} paired rows. v2 beats or ties RF on {n_beats_rf}/{n_total_rf} "
        f"reran rows. Catalytic gate: {n_pass}/{n_total_cat} kinetics pass. "
        f"Ki preservation: largest v2−v1 regression = {ki_max_regress:+.4f}."
    )
...
def main() -> int:
    ...
    if not os.path.exists(args.v2_csv):
        print(f"ERROR: {args.v2_csv} not found. Run analysis/aggregate_v2_results.py first.")
        return 2
    df = pd.read_csv(args.v2_csv)
    if df.empty:
        print("ERROR: v2 results CSV is empty.")
        return 2

    df = df.copy()
    df["verdict"] = df.apply(
        lambda r: row_verdict(r.get("v1_rmse"), r.get("v2_rmse"), r.get("rf_best_rmse")),
        axis=1,
    )

    per_task_cols = ["dataset_folder", "k_type", "split_mode", "embedding",
                     "v1_rmse", "v2_rmse", "rf_best_rmse",
                     "v2_minus_v1", "v2_minus_rf", "verdict"]
    ...
    md = [
        "# TRACE-Kin v2 improvement summary", "",
        f"Source: `{args.v2_csv}` ({len(df)} rows)", "",
        "## Headline", "",
        head, "",
        "## Per-task results", "",
        ...
        "## Legend", "",
        "- `✓ beats RF` — v2 RMSE ≤ RF best RMSE",
        f"- `~ ties RF` — v2 within {TIE_TOLERANCE} RMSE of RF best (above)",
        "- `✗ worse than RF` — v2 RMSE > RF best (and not within tie tolerance)",
        "- `✗ worse than v1` — v2 RMSE > v1 RMSE (regression vs. baseline)",
        "",
        "Per-kinetic verdict mirrors `analysis/promotion_gate.py`: PASS if ≥2/3 "
        "splits beat RF or the per-kinetic mean v2 RMSE ≤ mean RF RMSE.",
        "",
    ]
