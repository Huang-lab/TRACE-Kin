#!/usr/bin/env python
"""Decide whether to promote v3 results into the paper.

Gate (mechanical, defined in the project plan):

* For each catalytic kinetic in {kcat, km, kd, kcat_km}, count how many of its
  splits have v3 RMSE <= RF best RMSE. PASS the kinetic if 2 of 3 splits pass,
  OR if the per-kinetic mean v3 RMSE <= per-kinetic mean RF best.
* Overall gate PASSES if at least 3 of the 4 catalytic kinetics PASS.
* Separately: each Ki rerun must have v3 RMSE within +0.02 of v1 RMSE
...
KI_REGRESSION_TOLERANCE = 0.02  # v3 may not be more than 0.02 RMSE worse than v1 on Ki tasks
...
def evaluate_kinetic(kinetic_df: pd.DataFrame) -> tuple[bool, str]:
    ...
    valid = kinetic_df.dropna(subset=["v3_rmse", "rf_best_rmse"])
    if valid.empty:
        return False, "no v3 results vs RF available"
    valid = valid.copy()
    valid["beats_rf"] = valid["v3_rmse"] <= valid["rf_best_rmse"]
    n_total = len(valid)
    n_beats = int(valid["beats_rf"].sum())
    mean_v3 = valid["v3_rmse"].mean()
    mean_rf = valid["rf_best_rmse"].mean()
    by_count = (n_total >= 3 and n_beats >= 2) or (n_total < 3 and n_beats == n_total)
    by_mean = mean_v3 <= mean_rf
    passed = by_count or by_mean
    reasoning = (
        f"{n_beats}/{n_total} splits beat RF; "
        f"mean v3={mean_v3:.4f} vs mean RF={mean_rf:.4f}. "
...
def evaluate_ki_guards(ki_df: pd.DataFrame) -> tuple[bool, list[str]]:
    """Ki preservation guard: v3 within +0.02 of v1 on each Ki rerun row."""
    if ki_df.empty:
        return True, ["no Ki rows in v3 results — guard not exercised"]
    notes = []
    all_passed = True
    for _, row in ki_df.iterrows():
        if pd.isna(row["v3_rmse"]) or pd.isna(row["v1_rmse"]):
            notes.append(f"  - {row['dataset_folder']}: missing v3 or v1 (skipped)")
            continue
        delta = row["v3_rmse"] - row["v1_rmse"]
        ok = delta <= KI_REGRESSION_TOLERANCE
        notes.append(
            f"  - {row['dataset_folder']}: v3={row['v3_rmse']:.4f}, "
...
def main() -> None:
    ...
    if not os.path.exists(args.v3_csv):
        raise FileNotFoundError(f"v3 results not found: {args.v3_csv}")
    df = pd.read_csv(args.v3_csv)

    lines = ["# TRACE-Kin v3 Promotion Gate Decision", "",
             f"Source: `{args.v3_csv}` ({len(df)} v3 result rows)", ""]
    ...
    for k in CATALYTIC_KINETICS:
        ...
        if not sub.empty:
            for _, row in sub.iterrows():
                gap = (row["v3_rmse"] - row["rf_best_rmse"]) if pd.notna(row["rf_best_rmse"]) else None
                lines.append(
                    f"  - {row['dataset_folder']}: v3={row['v3_rmse']!s}, "
    ...
    lines.append(f"Tolerance: v3 may not be more than +{KI_REGRESSION_TOLERANCE:.2f} RMSE worse than v1 on Ki tasks.")
    ...
    if overall_pass:
        lines.append("- Proceed to Phase 8 packaging deliverables (figures, tables) using v3 weights.")
    else:
        lines.append("- Do NOT generate paper figures/tables yet.")
        if not catalytic_gate_pass:
            lines.append("- Catalytic gate failed: review per-kinetic breakdown above and iterate v3 design.")
        if not ki_passed:
            lines.append("- Ki guard failed: ship v1 weights for Ki tasks; v3 weights for everything else if catalytic gate passed.")
    lines.append("")
