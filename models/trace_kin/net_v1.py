...
class TraceKinV1(torch.nn.Module):
    def __init__(self, mol_deg, prot_deg,
                 ...
                 use_gated_prot_fusion=False,
                 deep_regression_head=False,
                 learnable_aux_loss=False,
                 device='cuda:0'):
        super(TraceKinV1, self).__init__()
        ...
        self.use_gated_prot_fusion = use_gated_prot_fusion
        self.deep_regression_head = deep_regression_head
        self.learnable_aux_loss = learnable_aux_loss
        ...
        if use_gated_prot_fusion:
            self.prot_fusion_gate = Linear(hidden_channels * 2, hidden_channels, bias=True)
        else:
            self.prot_fusion_gate = None
        ...
        if self.regression_head:
            if deep_regression_head:
                h = hidden_channels
                self.reg_out = MLP([h * 2, h * 2, h, max(h // 2, 1), 1])
            else:
        ...
        if learnable_aux_loss:
            self.loss_log_var_ortho = torch.nn.Parameter(torch.zeros(1))
            self.loss_log_var_cluster = torch.nn.Parameter(torch.zeros(1))
            if regression_head:
                self.loss_log_var_reg = torch.nn.Parameter(torch.zeros(1))
            else:
    ...
    def reset_parameters(self):
        ...
        if self.prot_fusion_gate is not None:
            self.prot_fusion_gate.reset_parameters()
    ...
    def forward(self,
                ...
                save_cluster = False,
                # v4 ChemBERT/MoLFormer embedding + per-residue AA index
                # (ignored by v1; accepted so the same trainer call site
                # works for both v1 and v4 without branching).
                chembert_fp=None,
                prot_aa_idx=None):
        # Init variables        
        reg_pred = None
        ...
        if self.use_gated_prot_fusion and self.prot_fusion_gate is not None:
            gate = torch.sigmoid(self.prot_fusion_gate(torch.cat([h_aa, h_evo], dim=-1)))
            residue_x = gate * h_aa + (1.0 - gate) * h_evo
        else:
            residue_x = h_aa + h_evo
