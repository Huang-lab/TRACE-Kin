...
class DrugProteinConv(MessagePassing):
    ...
    def forward(self, drug_x, clique_x, clique_batch, residue_x, edge_index: Adj):
        ...
        # Index drug_out with clique_batch to get drug features for each clique
        # drug_out has shape (batch_size, H * aC), clique_batch maps cliques to batch items
        # Ensure we get the right shape: (num_cliques, H * aC)
        drug_out_for_cliques = drug_out[clique_batch]  # Shape: (num_cliques, H * aC)
        # Handle case where drug_out might have singleton dimension
        if drug_out_for_cliques.dim() == 1:
            drug_out_for_cliques = drug_out_for_cliques.unsqueeze(0)
        # Ensure first dimension matches clique_x
        if drug_out_for_cliques.size(0) == 1 and clique_x.size(0) > 1:
            # Repeat the single drug feature for all cliques
            drug_out_for_cliques = drug_out_for_cliques.repeat(clique_x.size(0), 1)
        clique_out = torch.cat([clique_x, drug_out_for_cliques], dim=-1)
