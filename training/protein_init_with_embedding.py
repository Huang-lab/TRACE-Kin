...
def seq_feature(pro_seq):    
    # Replace known non-standard amino acids and invalid characters
    if 'U' in pro_seq or 'B' in pro_seq or 'O' in pro_seq or '0' in pro_seq:
        print('U, B, O, or 0 in Sequence')
    pro_seq = pro_seq.replace('U','X').replace('B','X').replace('O','X').replace('0','X')
    
    # Replace any remaining invalid characters with X (more robust approach)
    pro_seq = ''.join([aa if aa in pro_res_table else 'X' for aa in pro_seq])
...
def protein_init_with_embedding(seqs,
    ...
    for seq in tqdm(seqs):
        ...
        result_dict[seq] = {
            'seq': seq,
            'seq_feat': seq_feat_tensor,                 # 原来的氨基酸理化特征
            'token_representation': token_repr,          # 替换后的 embedding（来自 protein_features）
            'num_nodes': len(seq),
            'num_pos': torch.arange(len(seq)).reshape(-1, 1),
            'edge_index': edge_index,
            'edge_weight': edge_weight,
