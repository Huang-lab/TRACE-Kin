...
def minmax_norm(arr):
    """
    Safely apply min-max normalization.
    Handles the edge case where all values are identical (max == min),
    which would otherwise produce NaNs due to division by zero.
    """
    ...
    if isinstance(arr, torch.Tensor):
        ...
        # If all values are the same, return zeros instead of NaNs
        if (denom == 0).item():
