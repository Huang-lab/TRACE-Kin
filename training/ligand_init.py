...
class MoleculeGraphDataset():
    ...
    def mol_feature(self,mol):
        if mol is None:
            raise ValueError("Cannot compute mol_feature for None molecule")
        atom_ids = []
        atom_feats = []
    ...
    def mol_extra_feature(self, mol):
        if mol is None:
            raise ValueError("Cannot compute mol_extra_feature for None molecule")
    ...
    def mol_simplified_feature(self,mol):
        if mol is None:
            raise ValueError("Cannot compute mol_simplified_feature for None molecule")
        atom_ids = []
        atom_feats = []
        for atom in mol.GetAtoms():
            atom_ids.append(atom.GetIdx())
    ...
    def mol_sequence_simplified_feature(self,mol):
        if mol is None:
            raise ValueError("Cannot compute mol_sequence_simplified_feature for None molecule")
        atom_ids = []
        atom_feats = []
        for atom in mol.GetAtoms():
    ...
    def mol_full_feature(self, mol):
        if mol is None:
            raise ValueError("Cannot compute features for None molecule")
        atom_ids = []
        atom_feats = []
        for atom in mol.GetAtoms():
            atom_ids.append(atom.GetIdx())
    ...
    def bond_feature(self,mol):
        if mol is None:
            raise ValueError("Cannot compute bond_feature for None molecule")
        atom_num = len(mol.GetAtoms())
    ...
    def junction_tree(self,mol):
        if mol is None:
            raise ValueError("Cannot create junction tree for None molecule")
    ...
    def featurize(self,mol,type='atom_type'):
        if mol is None:
            raise ValueError("Cannot featurize None molecule")
        if type=='atom_type':
...
def smiles2graph(m_str):
    ...
    if mol is None:
        print(f"⚠️  Warning: Failed to parse SMILES: {m_str}")
        print(f"   Skipping this molecule and returning None")
        return None
