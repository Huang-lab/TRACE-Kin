...
def build_model(model_config: dict, mol_deg, prot_deg, device: str,
    ...
    if version == "v1":
        ...
        model = TraceKinV7(
            ...
            mol_pe_fold_norm = params.get('mol_pe_fold_norm', True),
            mol_pe_raw_norm = params.get('mol_pe_raw_norm', 'none'),
            mol_conv_type=params.get("mol_conv_type", "pna"),
            mol_grit_dim=params.get("mol_grit_dim", 64),
            mol_grit_layers=params.get("mol_grit_layers", 10),
            mol_grit_steps=params.get("mol_grit_steps", 21),
            mol_grit_dropout=params.get("mol_grit_dropout", 0.0),
            mol_grit_attn_dropout=params.get("mol_grit_attn_dropout", 0.2),
            n_cross_heads=params.get("n_cross_heads", 8),
            chembert_dim=params.get("chembert_dim", 768),
            dropout=params.get("dropout", 0.1),
            regression_head=tasks["regression_task"],
...
def train_one_member(args, model_config, swa_block, device, seed,
    ...
    engine = Trainer(
        ...
        warmup_frac=model_config["optimizer"].get("warmup_frac", None),
