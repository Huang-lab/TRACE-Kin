...
import json 
from .data_utils import unbatch
...
class Trainer(object):
    def __init__(self, model, lrate, min_lrate, wdecay, betas, eps, amsgrad, clip, steps_per_epoch, num_epochs, total_iters,
        ...
        # --- v2 SWA configuration -------------------------------------------------
        # When enabled, the trainer averages model weights across the final
        # ``swa_start_frac`` fraction of epochs (default 25%). The averaged
        # snapshot is used as the final checkpoint when SWA's running average
        # produces a better validation metric than the best per-epoch snapshot.
        # SWA gives RF-like ensembling for free, addressing one of the
        # mechanisms by which Random Forest beats PSICHIC on catalytic tasks.
        # GraphNorm is left alone (no ``update_bn`` pass): the running stats
        # accumulated during the SWA window are already representative.
        self.use_swa = bool(use_swa)
        self.swa_start_frac = float(swa_start_frac)
        self.swa_lr_factor = float(swa_lr_factor)
        ...
        if self.use_swa:
            ...
            self._swa_scheduler = SWALR(
                self.optimizer,
                swa_lr=self.lrate * self.swa_lr_factor,
                anneal_strategy='cos',
                anneal_epochs=max(1, int(self.num_epochs * 0.05) if self.num_epochs else 5),
            )
            self._swa_start_epoch = max(1, int(self.swa_start_frac * (self.num_epochs or 0)))
            print(
    ...
    def _log_schedule_summary(self):
        ...
        msg = (
            f"[LR schedule] schedule_lr={self.schedule_lr}, lrate={self.lrate}, "
            f"min_lrate={self.min_lrate}, warmup_iters={self.warmup_iters}, "
            f"lr_decay_iters={self.lr_decay_iters}, total_iters={self.total_iters}"
        )
        print(msg)
        try:
    ...
    def _compose_graph_reg_loss(self, sp_loss, o_loss_t, cl_loss_t, reg_loss_t=None, model=None):
        """Combine spectral (unused), ortho/cluster auxiliary losses, and optional regression MSE.

        When model.learnable_aux_loss, uses homoscedastic uncertainty weighting (Kendall et al.)
        so auxiliary terms do not overwhelm the primary regression objective.
        """
        model = model if model is not None else self.model
        if getattr(model, 'learnable_aux_loss', False):
            lv_o = model.loss_log_var_ortho
            lv_c = model.loss_log_var_cluster
            loss_val = sp_loss + (torch.exp(-lv_o) * o_loss_t + lv_o) + (torch.exp(-lv_c) * cl_loss_t + lv_c)
            if reg_loss_t is not None and getattr(model, 'loss_log_var_reg', None) is not None:
                lv_r = model.loss_log_var_reg
                loss_val = loss_val + (torch.exp(-lv_r) * reg_loss_t + lv_r)
            return loss_val
        loss_val = sp_loss + o_loss_t + cl_loss_t
        if reg_loss_t is not None:
            loss_val = loss_val + reg_loss_t * self.regression_weight
        return loss_val
        ...
        with output(initial_len=11, interval=0) as output_lines:
            for epoch in range(1, self.num_epochs+1):
                ...
                for data in train_loader:
                    ...
                    if curr_lr_rate < epoch_lr_min:
                        epoch_lr_min = curr_lr_rate
                    if curr_lr_rate > epoch_lr_max:
                        epoch_lr_max = curr_lr_rate
                ...
                lr_str = f'LR(min,max,end)=({epoch_lr_min:.3e}, {epoch_lr_max:.3e}, {curr_lr_rate:.3e}) iters_done={iter_num}/{self.total_iters}'
                output_lines[0] = ' '*30
                output_lines[1] = ' '*30
                output_lines[2] = '-'*40 
                ...
                # SWA: in the final fraction of training, accumulate the running
                # weight average and step the SWA scheduler. The averaging is
                # done after the optimizer's per-batch step completes for this
                # epoch, so it captures the post-update weights.
                if self.use_swa and epoch >= self._swa_start_epoch:
            ...
            # SWA finalization: if the averaged model evaluates better than the
            # best per-epoch checkpoint on the validation set, keep the SWA
            # weights. Otherwise discard them and stay with the best checkpoint.
            if self.use_swa and val_loader is not None:
                # Swap SWA averaged weights into self.model for evaluation.
                saved_state = {k: v.detach().clone() for k, v in self.model.state_dict().items()}
                swa_state = self._swa_model.module.state_dict()
                self.model.load_state_dict(swa_state, strict=True)
                swa_val_result = self.eval(val_loader)
                swa_metric = swa_val_result.get(self.evaluate_metric, None)
                higher_is_better = self.evaluate_metric not in ('rmse', 'mse', 'mae')
                swa_better = (
                    swa_metric is not None
                    and (
                        (higher_is_better and swa_metric > best_result)
                        or (not higher_is_better and swa_metric < best_result)
                    )
                )
                if swa_better:
                    print(
                        f"[SWA] keeping averaged weights: {self.evaluate_metric}={swa_metric:.4f} "
                        f"vs best per-epoch {best_result:.4f}"
                    )
                    torch.save(
                        self.model.state_dict(),
                        os.path.join(self.result_path, f'save_model_seed{self.runid}', 'model.pt'),
                    )
                    best_result = swa_metric
                    best_epoch = -1  # sentinel: means "SWA-averaged"
                    with open(self.result_path + '/full_result-{}.txt'.format(self.runid), 'a+') as f:
                        f.write(
                            f"[SWA] kept averaged weights: {self.evaluate_metric}={swa_metric:.4f}\n"
                        )
                else:
                    print(
                        f"[SWA] discarding averaged weights: {self.evaluate_metric}={swa_metric} "
                        f"vs best per-epoch {best_result}"
                    )
                    self.model.load_state_dict(saved_state, strict=True)
                    with open(self.result_path + '/full_result-{}.txt'.format(self.runid), 'a+') as f:
                        f.write(
                            f"[SWA] discarded averaged weights: {self.evaluate_metric}={swa_metric}\n"
                        )

            # Final summary of the best epoch chosen by the validation criterion.
            best_str = (
                f"[best epoch] epoch={best_epoch}, "
                f"best_{self.evaluate_metric}={best_result}, lr_at_best={best_epoch_lr:.3e}"
            )
            print(best_str)
            with open(self.result_path +'/full_result-{}.txt'.format(self.runid),'a+') as f:
                f.write(best_str + '\n')
            ...
            while iter_num < self.total_iters:
                for data in train_loader:
                    ...
                    if (iter_num % evaluate_step == 0) or (iter_num  == self.total_iters):
                        ...
                        if self.patience > 0 and evals_without_improvement >= self.patience:
                            print(f'\nEarly stopping at step {iter_num} (no improvement for {self.patience} evaluations)')
                            with open(self.result_path +'/full_result-{}.txt'.format(self.runid),'a+') as f:
                                f.write(f'\nEarly stopping at step {iter_num} (no improvement for {self.patience} evaluations)\n')
        ...
        with torch.no_grad():
            for data in tqdm(data_loader):
                ...
                if reg_pred is not None:
                    ...
                    reg_y = data.reg_y.squeeze().reshape(-1)
                    reg_loss_tensor = self.regression_loss(reg_pred, reg_y)
                    if not getattr(model, 'learnable_aux_loss', False):
                        reg_loss = (reg_loss_tensor * self.regression_weight).item()
                    else:
                        reg_loss = reg_loss_tensor.item()
                    reg_preds.append(reg_pred)
                ...
                loss_val = self._compose_graph_reg_loss(sp_loss, o_loss, cl_loss, reg_loss_tensor, model=model)
                sp_loss = sp_loss.item()
                o_loss = o_loss.item()
                cl_loss = cl_loss.item()
